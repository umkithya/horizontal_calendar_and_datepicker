...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
  ...
  int _calculateFocusedPage(
    ...
    switch (format) {
      ...
      case CalendarFormat.week:
        return _getDayDate(startDay, focusedDay);
      default:
        return _getMonthCount(startDay, focusedDay);
    }
  }
  ...
  int _getDayDate(DateTime first, DateTime last) {
    // if (dayDif >= 7) {
    debugPrint("Hello ${last.difference(_firstDayOfWeek(first)).inDays ~/ 7}");
    return last.difference(_firstDayOfWeek(first)).inDays ~/ 7;
    // } else {
    //   debugPrint("");
    //   return 0;
  ...
  void _showCustomPicker() async {
    ...
    if (Platform.isAndroid) {
      ...
      if (pickedDate != null) {
        ...
        _updateFocusOnTap(temp);
        monthindex = temp.month;
        widget.onDaySelected?.call(temp, _focusedDay!.value);
      } else {}
    } else {
                            ...
                            child: Container(
                              ...
                              child: Center(
                                child: Text(
                                  "Confirm",
                                  style:
                                      widget.calendarStyle.confirmButtonStyle,
                                ),
                              ),
                            ),
                          ),
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        child: Center(
                          child: Text(
                            "Cancel",
                            style: widget.calendarStyle.cancelButtonStyle,
                          ),
                        ),
                      ),
                    ),
  ...
  Widget _buildCell(DateTime day, DateTime focusedDay, value) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        var isSelected = widget.selectedDayPredicate?.call(day);
        ...
        if (dowCell == null) {
          ...
          dowCell = Center(
            child: ExcludeSemantics(
              child: Text(
                "${weekdayString[0].toUpperCase()}${weekdayString.substring(1).toLowerCase()}",
                style: isWeekend
